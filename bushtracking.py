...
class Bush: 
    ...
    def count_berries(self, img:cv2.Mat, berry_model:YOLO) -> tuple[int, cv2.Mat]:
        ...
        for i in range(len(X)-1):
            ...
            for j in range(len(Y)-1):
                ...
                results = berry_model.predict(tile)
                try:
                    ...
                    xyxy = []
                    clss = []
                for berry in zip(xyxy, clss):
                    ([left, top, rite, btm], cls) = berry
        ...
        return berry_count, annotated_img
...
def main(source:str, kwargs:dict):
    ...
    models = {m:[w for w in weights if m in w][0] for m in ('bush', 'berry')}
    bush_model = YOLO(f'weights/{models["bush"]}')
    berry_model = YOLO(f'weights/{models["berry"]}')
    ...
    for name, i in zip(names, rownumbers):
        csv_data += do_tracking(source, name, i, (bush_model, berry_model), kwargs)
            ...
            for id in registered_ids:
                ...
                if bush.updated:
                    if bush.crossed(WIDTH,HEIGHT,direction):
                        # add bush to tally, get berry count, 
                        # collect csv data, save frame with bush and berry annotations
                        bush_count += 1
                        (left, top, rite, btm) = bush.frames[-1]
                        # thnx luke
                        img = frame.copy()
                        berry_count, img[top:btm, left:rite] = bush.count_berries(img[top:btm, left:rite], berry_model)
                        # thnx jake
                        ...
                        csv_data.append( [img_name, frame_count, id, bush_count, row_number, lat, lng, berry_count] )
                        if save_frames:
